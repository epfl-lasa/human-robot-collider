...
human_adult_ID = p.loadURDF("../data/human_adult.urdf",
	flags=p.URDF_MAINTAIN_LINK_ORDER)

#the following file is from https://motion-database.humanoids.kit.edu/details/motions/961/
tree = ET.parse('../data/walk_neutral05.xml')
root = tree.getroot()


time.sleep(1.0)

joint_position_trajectories = np.zeros([len(root[0][3]), 37])
...
for t in range(len(root[0][3])):
	...
	applyMMMRotationToURDFJoint(human_adult_ID, 13,
		...
		float(value_string_list[20]))

	# right arm to right forearm
	p.resetJointState(human_adult_ID, 14, -float(value_string_list[26]))
	# left arm to left forearm
	p.resetJointState(human_adult_ID, 15, -float(value_string_list[12]))

	# right_forearm_to_right_hand
	applyMMMRotationToURDFJoint(human_adult_ID, 16,
		float(value_string_list[35]),
		float(value_string_list[36]),
		0.0)
